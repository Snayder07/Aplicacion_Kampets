...
public class PanelAdminCitas {
    ...
    private JPanel crearContenido() {
        ...
        // ── Filtros: solo los relevantes ──────────────────────
        // "En espera" y "Pendiente" son similares → dejamos solo "Pendientes"
        ...
        for (int i = 0; i < fnombres.length; i++) {
            ...
            // Contar citas de este estado para mostrar badge
    ...
    private JPanel crearTabla(String filtro, JPanel bodyRef) {
        ...
        for (Citas cita : todasCitas) {
            if (filtro.equals("Todas")) {
                // Excluir canceladas del listado "Todas" para no mezclar
        ...
        // Columnas — la última es "Acciones" (no viene de BD)
        String[] cols = {"Mascota", "Dueño", "Veterinario", "Fecha", "Hora", "Estado", "Acción"};
        ...
        for (int i = 0; i < citasFiltradas.size(); i++) {
            ...
            datos[i][1] = duenio;
            datos[i][2] = cita.getEmpleado()  != null ? cita.getEmpleado().getNombre()  : "—";
            ...
            datos[i][6] = cita.getId(); // ID para identificar la fila en los botones
        }
        ...
        // ── Renderer base para demás columnas ─────────────────
    ...
    // ════════════════════════════════════════════════════════
    //  RENDERER columna Acción — muestra botón visual
    // ════════════════════════════════════════════════════════
    ...
    // ════════════════════════════════════════════════════════
    //  EDITOR columna Acción — ejecuta la acción al hacer clic
    // ════════════════════════════════════════════════════════
        ...
        AccionEditor(JTable tabla, List<Citas> citas, JPanel bodyRef) {
            ...
            btnConfirmar.addActionListener(e -> {
                ...
                if (respuesta == JOptionPane.YES_OPTION) {
                    ...
                    construir(); // recargar panel completo
                }
            });
        }
